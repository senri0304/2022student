...
# fixation point
def fixation(d, v):
    d.rectangle((int(sz / 2) + eccentricity*v - f, int(sz / 2) + eccentricity*v + f * 3,
                 int(sz / 2) + eccentricity*v + f, int(sz / 2) + eccentricity*v - f * 3),
                fill=(0, 0, 255), outline=None)
    d.rectangle((int(sz / 2) + eccentricity*v - f * 3, int(sz / 2) + eccentricity*v + f,
                 int(sz / 2) + eccentricity*v + f * 3, int(sz / 2) + eccentricity*v - f),
                fill=(0, 0, 255), outline=None)
...
def ls(ev):
    ...
    basename = os.path.basename('ls' + str(ev) + '.png')
...
def dot(d, n):
    for i in range(n+1):
        d.rectangle((int(sz / 2) - int((ll / 2)) - (f*3)*i + f, int(sz / 2) + int(f / 2),
                     int(sz / 2) - int((ll / 2)) - (f*3)*i, int(sz / 2) - int(f / 2)),
...
def line_resize(l, ev):
    ...
    basename = os.path.basename(str(l) + str(ev) + 'ls.png')
    img.save(os.path.join(to_dir, basename), quality=100)
