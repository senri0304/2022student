...
def set_polygon(seq, seq2, seq3):
    ...
    if seq3 == -1:
        ...
        R = pyglet.resource.image('stereograms/' + str(seq2) + 'rds' + str(seq) + 'R.png')
        R = pyglet.sprite.Sprite(R)
        R.x = cntx + deg1 * iso + cal - R.width / 2.0
        R.y = cnty - R.height / 2.0
        L = pyglet.resource.image('stereograms/' + str(seq2) + 'rds' + str(seq) + 'L.png')
...
def prepare_routine():
    if n < len(variation2):
        fixer()
        set_polygon(sequence[n], sequence2[n], test_eye)
    else:
        pass
...
fixer()
set_polygon(sequence[0], sequence2[0], test_eye)
